...
<div class="row">
  <div class="col-md-6">
    <div class="card mb-4">
      <div class="card-body">
        <textarea id="card-input" placeholder="4411040534646642|07|2028|200" rows="8" class="form-control text-center mb-2"></textarea>

        <select id="gate" class="form-control mb-2">
          <option value="./Gate/autohitter.php">AUTO HITTER </option>
        </select>
        
        <input type="text" id="invoiceInput" class="form-control mb-2" placeholder="Enter invoice URL">
        <input type="text" id="proxyInput" class="form-control mb-2" placeholder="Enter proxy (user:pass@ip:port)">

        <div class="d-flex justify-content-between mb-3">
          <button class="btn btn-secondary w-100 me-1" id="clearSiteBtn">Clear Site</button>
        </div>
  ...
  <div class="col-md-12">
    <div class="card mb-4">
      <div class="card-body">
        ...
        <div id="resultList" class="results-list"></div>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  window.onload = () => {
    const savedSite = localStorage.getItem('customSite');
    if (savedSite) customSiteInput.value = savedSite;
  };

  customSiteInput.addEventListener('input', () => {
    localStorage.setItem('customSite', customSiteInput.value.trim());
  });

  clearSiteBtn.addEventListener('click', () => {
    localStorage.removeItem('customSite');
  ...
  startBtn.addEventListener('click', async () => {
    ...
    if (!gate || !invoice || !proxy) {
      alert('Please fill in gateway, invoice, and proxy.');
      return;
    }

    carregadas.textContent = cards.length;
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      try {
        ...
        const line = `[${data.gateway || 'N/A'}] ${data.card || cc} => ${data.status || 'UNKNOWN'} | ${data.message || 'No message'} (${data.time || '0s'})`;

        resultList.innerHTML += `<div>${line}</div>`;
      } catch (err) {
        resultList.innerHTML += `<div class="text-warning">${cc} => Request failed</div>`;
      }
